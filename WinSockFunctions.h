...
// #pragma comment (lib, "Ws2_32.lib"). Commented out for MinGW Compiler, Windows socket libraries are linked in CMakeLists.txt
// #pragma comment (lib, "Mswsock.lib"). Commented out for MinGW Compiler, Windows socket libraries are linked in CMakeLists.txt
// #pragma comment (lib, "AdvApi32.lib"). Commented out for MinGW Compiler, Windows socket libraries are linked in CMakeLists.txt
...
namespace WindowsSocketApp {
    ...
    inline bool resolve_address_and_port (const char *nodename, const char *servname,const struct addrinfo *hints,struct addrinfo **res) {
    ...
    inline bool receive_until_empty_input(SOCKET sender_socket, std::vector<char> &recv_buffer) {
        int i_receive_result {-1};
        do {
            i_receive_result = recv(sender_socket, recv_buffer.data(), static_cast<int>(recv_buffer.size()), 0);
    ...
    inline bool send_buffer_content (SOCKET receiver_socket, const std::vector<char> &buffer) {
        auto i_send_result = send(receiver_socket, buffer.data(), static_cast<int>(buffer.size()), 0 );
    ...
    inline bool send_buffer_content (SOCKET receiver_socket, const std::string &buffer) {
        auto i_send_result = send(receiver_socket, buffer.c_str(), static_cast<int>(buffer.size()), 0 );
        if (i_send_result == SOCKET_ERROR) {
            std::cout << "send failed with error: " << WSAGetLastError() << std::endl;
            return false;
        }
