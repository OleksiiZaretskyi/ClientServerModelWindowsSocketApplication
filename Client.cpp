...
namespace WindowsSocketApp {
    ...
    void Client::connect_to_server() {
        if (!resolve_address_and_port(server_ip_.c_str(), port_.c_str(), &hints_, &result_)) {
            socket_cleanup();
            return;
        } else {
        ...
        for(addrinfo_ptr_=result_; addrinfo_ptr_ != nullptr ;addrinfo_ptr_=addrinfo_ptr_->ai_next) {

            connect_socket_ = create_socket(addrinfo_ptr_->ai_family, addrinfo_ptr_->ai_socktype, addrinfo_ptr_->ai_protocol);
            if (connect_socket_ == INVALID_SOCKET) {
                socket_cleanup();
                return;
            }

            if (connect(connect_socket_, addrinfo_ptr_->ai_addr, static_cast<int>(addrinfo_ptr_->ai_addrlen)) == SOCKET_ERROR) {
    ...
    void Client::shutdown_message_sending() {
        if (!shutdown_sending_side(connect_socket_)) {
            ...
            client_initialization_status_ = Client_Initialization_Status::SHUTDOWN_FOR_SENDING;
        }
    }
    ...
    void Client::close_connection_to_server() {
        socket_cleanup();
    }

    void Client::socket_cleanup() {
        std::cout << "Closing all client sockets, freeing address info, closing connection to server... " << std::endl;
    ...
    const char * Client::get_port() const {
        return port_.c_str();
    }
    ...
    const char * Client::get_server_ip() const {
