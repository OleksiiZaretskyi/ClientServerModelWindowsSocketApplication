...
int main() {
    ...
    if (!WindowsSocketApp::initialize_winsock_2_0(wsaData)) {
        std::cout << "Failed to initialize Winsock 2.0. Exiting from main()..." << std::endl;
        return -1;
    }
    ...
    if (new_server.get_server_init_status() == WindowsSocketApp::Server_Initialization_Status::CLIENT_CONNECTION_HANDLED &&
        new_client.get_client_init_status() == WindowsSocketApp::Client_Initialization_Status::CONNECTED) {

        std::cout << "Connection between Server and Client established. Starting message exchange..." << std::endl;
        ...
        std::cout << "\n=== Client receiving message analytics from server ===" << std::endl;
        ...
        std::cout << "\nFailed to establish connection between server and client." << std::endl;
    }
