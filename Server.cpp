...
namespace WindowsSocketApp {
    ...
    void Server::start_server() {
        ...
        listen_socket_ = create_socket(result_->ai_family, result_->ai_socktype, result_->ai_protocol);
        if (listen_socket_ == INVALID_SOCKET) {
            socket_cleanup();
            return;
        }

        if (!bind_socket(listen_socket_, result_->ai_addr, static_cast<int>(result_->ai_addrlen))) {
            socket_cleanup();
            return;
        } else {
        ...
        if (!listen_on_socket(listen_socket_, maximum_pending_connections)) {
            ...
            server_initialization_status_ = Server_Initialization_Status::LISTENING_FOR_CONNECTION;
    ...
    void Server::accept_connections() {
        ...
        if (client_socket_ == INVALID_SOCKET) {
            ...
            server_initialization_status_ = Server_Initialization_Status::CLIENT_CONNECTION_HANDLED;
            std::cout << "Client connection handled for client socket: " << client_socket_ << std::endl;
            // No longer need the server socket in this particular single-thread example, should be re-factored when introducing the multi-threading later
    ...
    void Server::calculate_recv_message_analytics() {
        ...
        recv_message_analytics_ = "Received message analytics: Length: " + std::to_string(recv_message_length)
    ...
    void Server::shutdown_message_sending() {
        if (!shutdown_sending_side(client_socket_)) {
            ...
            server_initialization_status_ = Server_Initialization_Status::SHUTDOWN_FOR_SENDING;
        }
    }
    ...
    void Server::socket_cleanup() {
        std::cout << "Closing all server sockets, freeing address info, stopping the server... " << std::endl;
